    ...
    return (
        <div className="Page overflow-hidden">
            <div className="w-full h-fit flex flex-row items-center justify-evenly text-neutral-200 p-10 ">
                ...
                <Separator orientation="vertical" className="h-full w-[1px] bg-neutral-600 mx-4"/>
                <div className="flex flex-col group flex-grow cursor-pointer" onClick={()=>setTab("Robotics")}>
                    <motion.span 
                    initial={{ opacity: 0, x: 50 }}
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ duration: 0.5, ease: "easeInOut", delay: 0.2 }}
                    className={`${Tab === "Robotics"?"text-center":""}`}>Robotics</motion.span>
                    <span className={`h-[1px] w-0 ${Tab === "Robotics"?"w-full":"group-hover:w-full"} bg-neutral-300 transition-all duration-300`}></span>
                </div>
                <Separator orientation="vertical" className="h-full w-[1px] bg-neutral-600 mx-4"/>
